...
def transform_score(
    ...
    offs = (
        torch.arange(kernel_size // kernel_stride)[:, None] + torch.arange(block_size // kernel_stride)[None, :]
    ).view(-1)
    offs = dict(Counter(offs.tolist()))
    for k, v in offs.items():
        block_score[..., :full_blocks] += v * score[..., k :: block_size // kernel_stride][..., :full_blocks]
    # set init block and local block score
    batch_size = cu_seqlens_q.shape[0] - 1
    q_idx = torch.cat(
        [torch.arange(cu_seqlens_q[i + 1] - cu_seqlens_q[i], device=score.device) for i in range(batch_size)],
        dim=0,
    )
    q_idx = q_idx // block_size
    b_idx = torch.arange(max_blocks, device=score.device)
    block_score[..., :init_blocks] = torch.inf
    local_mask = (q_idx[:, None] >= b_idx[None, :]) & (q_idx[:, None] < b_idx[None, :] + local_blocks)
...
def compressed_attention_torch(
    ...
    # get mask
    mask = torch.zeros((total_query_len, total_key_len), dtype=torch.bool, device=q.device)
    for b in range(batch_size):
        q_len, k_len = (
            cu_seqlens_q[b + 1] - cu_seqlens_q[b],
            cu_seqlens_k[b + 1] - cu_seqlens_k[b],
        )
        k_max_ids = torch.arange(k_len, device=q.device) * kernel_stride + kernel_size - 1
        q_ids = torch.arange(q_len, device=q.device)
        mask[cu_seqlens_q[b] : cu_seqlens_q[b + 1], cu_seqlens_k[b] : cu_seqlens_k[b + 1]] = (
            q_ids[:, None] >= k_max_ids[None, :]
        )
    # attention
    qk = torch.einsum("qhd,khd->hqk", q, k.repeat_interleave(num_share_q_heads, 1)) * sm_scale
    ...
    qk = qk.nan_to_num(0)
    attn_output = torch.einsum("hqk,khd->qhd", qk.to(v.dtype), v.repeat_interleave(num_share_q_heads, 1))
    with torch.no_grad():

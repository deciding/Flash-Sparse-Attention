...
class NativeSparseAttentionNoRoPE(torch.nn.Module):
    def __init__(
        ...
        # self.proj_qkv = torch.nn.Linear(
        #     self.hidden_size, 3 * self.num_kv_heads * self.head_dim, bias=False
        # )
        ...
        self.compress_key = torch.nn.Parameter(
            torch.zeros(self.num_kv_heads * self.head_dim, self.head_dim, self.kernel_size)
        )
    ...
    def forward(
        ...
        sparse_attn_output = topk_sparse_attention(q, k, v, topk_idx, self.block_size, cu_seqlens, None)

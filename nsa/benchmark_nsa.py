...
from nsa.parallel import parallel_nsa, parallel_nsa_compression_fwd
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        x_vals=[1024 * 2 ** i for i in range(5, 6)],
...
def benchmark(T, provider):
    ...
    g_cmp = torch.rand((B, T, HQ), dtype=dtype, device='cuda').requires_grad_(requires_grad)
    g_slc = torch.rand((B, T, HQ), dtype=dtype, device='cuda').requires_grad_(requires_grad)
    g_swa = torch.rand((B, T, HQ), dtype=dtype, device='cuda').requires_grad_(requires_grad)
    ...
    proton.start('nsa', hook='triton')
    #proton.deactivate()
    #parallel_nsa(q, k, v, None, g_slc, g_swa, block_indices, block_counts, block_size, window_size)
    parallel_nsa(q, k, v, g_cmp, g_slc, g_swa, block_indices, block_counts, block_size, window_size)
    proton.finalize()

    import triton.profiler.viewer as proton_viewer
    metric_names = ["time/ms"]
    file_name = f"nsa.hatchet"

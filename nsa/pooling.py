...
from nsa.common_utils import prepare_chunk_indices
from nsa.utils import autocast_custom_bwd, autocast_custom_fwd, input_guard
...
@triton.autotune(
    configs=[
        triton.Config({'BD': BD}, num_warps=num_warps, num_stages=3)
        for BD in [64]
        for num_warps in [1]
    ],
    key=['BT']
...
def mean_pooling_fwd(
    ...
    with proton.scope("pooling_inner"):
